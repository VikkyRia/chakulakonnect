...
function AddProduct() {
    ...
    return (
        <div className="flex-1 flex flex-col h-full overflow-y-auto">
            {/* Top Navigation Bar */}
            <header className="bg-white/80 backdrop-blur-xl border-b border-slate-200/60 h-20 flex items-center justify-between px-4 sm:px-8 lg:px-12 sticky top-0 z-50 shrink-0">
                <div className="flex items-center gap-3 sm:gap-6">
                    ...
                    <div className="flex flex-col">
                        <span className="text-[8px] sm:text-[10px] font-black text-slate-400 uppercase tracking-widest leading-none mb-1">Marketplace</span>
                        <h1 className="text-sm sm:text-lg font-black text-slate-900 leading-none">Add New Product</h1>
                    </div>
                </div>
                ...
                <div className="flex items-center gap-2 sm:gap-4">
                    <button
                        type="button"
                        onClick={() => navigate('/seller-dashboard/list')}
                        className="hidden xs:block px-4 sm:px-6 py-2.5 rounded-xl text-slate-500 font-bold hover:bg-slate-100 transition-all text-xs"
                    >
                        Discard
                    </button>
                    <button
                        ...
                        disabled={isLoading}
                        className="px-4 sm:px-8 py-2.5 rounded-xl bg-emerald-500 text-white font-black hover:bg-emerald-600 shadow-xl shadow-emerald-200/50 transition-all flex items-center gap-2 group active:scale-95 disabled:opacity-50 text-[10px] sm:text-xs"
                    >
                        {isLoading ? <Loader2 className="animate-spin" size={16} /> : <Plus size={16} />}
                        <span className="hidden sm:inline">Publish Item</span>
